...
static void draw_menu() {
    ...
    if (total == 0) {
        text_draw(0, 24, "No MIDI.");
        text_draw(0, 44, "Connect PC ->add");          // PCで /midi に曲を追加
        display.display();
        return;
    }
...
#if USE_USB_MSC
// PC接続中(USBドライブとして使用中)の表示
static void draw_pc_connected() {
    display.clearDisplay();
    text_set_color(SSD1306_WHITE);
    display.drawRect(0, 0, CFG_PHYS_W, CFG_PHYS_H, SSD1306_WHITE);
    const char* l1 = "Connected";
    const char* l2 = "PC";
    text_draw((CFG_PHYS_W - text_width(l1)) / 2, 16, l1);
    text_draw((CFG_PHYS_W - text_width(l2)) / 2, 36, l2);
    display.display();
}
#endif
...
#if USE_USB_MIDI
// 外部MIDI音源モードの表示
static void draw_usb_midi() {
    display.clearDisplay();
    text_set_color(SSD1306_WHITE);
    display.drawRect(0, 0, CFG_PHYS_W, CFG_PHYS_H, SSD1306_WHITE);
    const char* l1 = "USB MIDI";
    const char* l2 = "Sound Module";
    text_draw((CFG_PHYS_W - text_width(l1)) / 2, 14, l1);
    text_draw((CFG_PHYS_W - text_width(l2)) / 2, 36, l2);
    display.display();
}
#endif
...
#if USE_USB_MSC
    usb_set_fs_busy(true);   // ファイルを開いた→PCマウントを保留させる
#endif
...
void setup() {
    ...
    if (!usb_storage_begin()) {
        DBG_PRINTLN("[fs] FAIL");
        display.clearDisplay(); text_set_color(SSD1306_WHITE);
        text_draw(0,24,"FS init FAIL"); display.display();
        while(1) delay(1000);
    }
    ...
    // 0曲でも停止しない: PCを接続して /midi に曲を入れてもらう
    if (n > 0 && CFG_AUTO_START) start_song(0);
...
void loop() {
    ...
    bool conn = usb_pc_connected();

    // 再生/曲ロード中はFSを占有 → PCのマウントを保留させる
    if (!conn) usb_set_fs_busy(g_player.is_playing() || g_showing_info);

    // 再生中にPCがマウントを要求 → 停止してFSを明け渡す (次の試行で許可される)
    if (usb_take_wants_mount()) {
        if (g_player.is_playing()) g_player.stop();
        g_showing_info = false;
        ps_all_off();
        usb_set_fs_busy(false);
    }

    if (conn) {
        if (!s_was_conn) {                       // 接続開始: 停止して "Connected PC"
            if (g_player.is_playing()) g_player.stop();
            g_showing_info = false;
            ps_all_off();
            draw_pc_connected();
        ...
        s_was_conn = true;
        // 無音を流し続けてDMAを止めない(FS/再生には触れない)
        if (ps_fill_seq - ps_cons_seq < 2) ps_paint_palette(now);
        return;
    }
    s_was_conn = false;

    // 取り外し: songlist を再スキャンしてメニューへ
    if (usb_take_unplugged()) {
        g_songlist.load(CFG_MIDI_DIR);
        g_song_idx = 0; g_in_menu = true;
        draw_menu();
    }
#endif
...
#if USE_USB_MIDI
    // ===== 外部MIDI音源モード =====
    usb_midi_poll(now);                          // 受信を取り込む(常時)
    static bool s_midi_mode = false;
    if (usb_midi_is_active()) {
        if (!s_midi_mode) {                      // 突入: ファイル再生を止めて画面表示
            if (g_player.is_playing()) g_player.stop();
            g_showing_info = false; g_in_menu = false;
            usb_midi_all_off();
            draw_usb_midi();
        }
        s_midi_mode = true;
        if (ps_fill_seq - ps_cons_seq < 2) { ps_paint_palette(now); g_paint_count++; }
        return;                                  // 受信MIDIの発音に専念
    } else if (s_midi_mode) {                     // 退出: 一定時間MIDIが来ない → メニューへ
        s_midi_mode = false;
        usb_midi_all_off();
        g_in_menu = true;
        draw_menu();
    ...
    if (g_player.is_finished()) {
        // 残ノーツ対策: 自然に消えるのを待つが、1.5秒経っても残っていれば強制消音。
        static uint32_t fin_t = 0;
        if (fin_t == 0) fin_t = now ? now : 1;
        if (ps_active == 0 || (now - fin_t) > 1500) {
            ps_all_off();                       // 曲終了時に確実に全消音
            fin_t = 0;
            int next = (g_song_idx+1) % g_songlist.count();
            if (!CFG_REPEAT_MODE && next==0) {
                display.clearDisplay(); text_set_color(SSD1306_WHITE);
                text_draw(0,24,"Finished."); display.display();
                g_player.stop();
                return;
            }
            start_song(next);
